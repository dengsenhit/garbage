...
wire                            ycbcr_de;
///////need change
wire [31:0] read_data_32bit;
wire [7:0]  wr_burst_mask;
...
assign hdmi_hs = per_frame_href_Erosion;
assign hdmi_vs = per_frame_vsync_Erosion;
assign hdmi_de = per_frame_clken_Erosion;
// assign hdmi_hs = hs;
// assign hdmi_vs = vs;
// assign hdmi_de = de;
assign hdmi_r  = vga_data[23:16];
assign hdmi_g  = vga_data[15:8];
assign hdmi_b  = vga_data[7:0];
// assign hdmi_r  = ycbcr_y2;
// assign hdmi_g  = ycbcr_y2;
// assign hdmi_b  = ycbcr_y2;
...
rgb_to_ycbcr_new rgb_to_ycbcr_m1(
	.clk                        (video_clk                ),
	.rst                        (~done_init               ),
	.rgb_r1                      ({vout_data[15:11],3'd0}  ),
	.rgb_g1                      ({vout_data[10:5],2'd0}   ),
	.rgb_b1                      ({vout_data[4:0],3'd0}    ),
	.rgb_hs                     (hs                       ),
	.rgb_vs                     (vs                       ),
	.rgb_de                     (de                       ),
...
rgb_to_ycbcr_new rgb_to_ycbcr_m2(
	.clk                        (video_clk                ),
	.rst                        (~done_init               ),
	.rgb_r2                      ({vout_data[31:27],3'd0}  ),
	.rgb_g2                      ({vout_data[26:21],2'd0}   ),
	.rgb_b2                      ({vout_data[20:16],3'd0}    ),
	.rgb_hs                     (hs                       ),
	.rgb_vs                     (vs                       ),
	.rgb_de                     (de                       ),
//	 .cmos_mask_data             (cmos_mask_data           ),
...
// wire af_filter_hs;
// wire af_filter_de;
// wire [7:0] af_filter_y1;
// wire [7:0] af_filter_y2;
// VIP_Gray_Mean_Filter u_VIP_Gray_Mean_Filter1(
	// //global clock
	// .clk(video_clk),  				//100MHz
	// .rst_n(rst_n&&done_init),				//global reset

	// //Image data prepred to be processd
	// .per_frame_vsync(ycbcr_vs),	//Prepared Image data vsync valid signal
	// .per_frame_href(ycbcr_hs),		//Prepared Image data href vaild  signal
	// .per_frame_clken(ycbcr_de),	//Prepared Image data output/capture enable clock
	// .per_img_Y(ycbcr_y1),			//Prepared Image brightness input
	
	// //Image data has been processd
	// .post_frame_vsync(af_filter_vs),	//Processed Image data vsync valid signal
	// .post_frame_href(af_filter_hs),	//Processed Image data href vaild  signal
	// .post_frame_clken(af_filter_de),	//Processed Image data output/capture enable clock
	// .post_img_Y(af_filter_y1)			//Processed Image brightness input
// );

// VIP_Gray_Mean_Filter u_VIP_Gray_Mean_Filter2(
	// //global clock
	// .clk(video_clk),  				//100MHz
	// .rst_n(rst_n&&done_init),				//global reset
	...
	// .per_frame_href(ycbcr_hs),		//Prepared Image data href vaild  signal
	// .per_frame_clken(ycbcr_de),	//Prepared Image data output/capture enable clock
	// .per_img_Y(ycbcr_y2),			//Prepared Image brightness input
	...
	// .post_img_Y(af_filter_y2)			//Processed Image brightness input
// );


assign cmos_mask_data={ycbcr_y1,ycbcr_y2};
// // /******************************below write by myself**************************************************************/
// wire per_frame_clken;
// wire per_frame_href;
// wire per_frame_vsync;
// wire [15:0]	vga_data_rgb;
wire [7:0]	img1_data ;
wire [7:0]	img2_data ;
// // wire [7:0]	post_img1_data ;
// // wire [7:0]	post_img2_data ;
// wire [10:0]	hcnt ;
// wire [9:0]	vcnt ;
// vga_driver	vga_driver_inst(
	// .clk		 		(video_clk			),		//65Mhz
	// .rst_n				(rst_n&&done_init),
	// .img_data			(cmos_mask_data		),
	// .img_data_req		(ycbcr_de    	),
	// .data_vaild			(per_frame_clken	),
	// .vcnt				(vcnt				),
	// .hcnt				(hcnt				),
	// .hsync				(per_frame_href     ),
	// .vsync				(per_frame_vsync    ),
	// .vga_data_rgb		(vga_data_rgb		),
// //	.lcd_blank			(lcd_blank			),
// //	.lcd_dclk           (lcd_dclk			),
	// .vga_data       	(img1_data   	    ),
	// .vga_data2			(img2_data			)
	// );
assign  img2_data =(ycbcr_de)? cmos_mask_data[7:0]: 8'b0;
assign  img1_data =(ycbcr_de)? cmos_mask_data[15:8]: 8'b0;
wire BIT_IMG ;
assign 	BIT_IMG = (((img2_data - img1_data)>GRAY_THRESHOLD))? 1'b1 : 1'b0;  //ãÐÖµ
...
VIP_Bit_Dilation_Detector u_VIP_Bit_Dilation_Detector (
    .clk(video_clk), 
    .rst_n(rst_n&&done_init), 
    .per_frame_vsync(ycbcr_vs), 
    .per_frame_href(ycbcr_hs), 
...
// (* KEEP="TRUE"*) wire 	post_img_Bit_Erosion2 ; 
wire vga_bit;
VIP_Bit_Erosion_Detector u2_VIP_Bit_Erosion_Detector (
    .clk(video_clk), 
    .rst_n(rst_n&&done_init), 
    .per_frame_vsync(per_frame_vsync_Erosion), 
    .per_frame_href(per_frame_href_Erosion), 
    .per_frame_clken(per_frame_clken_Erosion), 
    .per_img_Bit(~BIT), 
    .post_frame_vsync(), 
    .post_frame_href(), 
...
wire [23:0] vga_data;
assign vga_data = (vidon)?{img1_data,img1_data,img1_data}:24'b111111110000000000000000;
...
// // assign vga_data = (vidon)?vga_data_rgb:16'b1111100000000000;
// // assign vga_data = (vidon)?16'b1111111111111111:16'b1111100000000000;
/******************************above write by myself**************************************************************/
// sobel sobel_m0(
	// .rst                        (~done_init               ),
	// .pclk                       (video_clk                ),
	// .threshold                  (8'd40                    ),
...
(
	...
	.read_addr_index            (read_addr_index          ),
	.read_len                   (24'd393214              ),//frame size 
	.read_en                    (read_en                  ),
	.read_data                  (read_data                ),
    .read_data_32bit            (read_data_32bit),
	...
	.write_addr_index           (write_addr_index         ),
	.write_len                  (24'd393214          ), //frame size  
...
aq_axi_master_new u_aq_axi_master         
	(
